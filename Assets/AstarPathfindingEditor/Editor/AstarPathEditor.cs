...
public class AstarPathEditor : Editor {
	...
	public static string CRecastHelpURL = "http://www.arongranberg.com/astar/docs/class_pathfinding_1_1_recast_graph.html#a2d3655da3ed281674cf5125205e2a246.php";
	public static readonly string AstarProTooltip = "A* Pathfinding Project Pro only feature\nThe Pro version can soon be bought on the A* Pathfinding Project homepage,";
	...
	public static double updateCheckRate = 1F;
	...
	public void GetAstarEditorSettings () {
		...
		useDarkSkin = EditorPrefs.GetInt ("UseDarkSkin",0) == 1;
	...
	public void EnableJs () {
		...
		AssetDatabase.MoveAsset ("Assets/AstarPathfindingProject/Editor","Assets/AstarPathfindingEditor/Editor");
		AssetDatabase.MoveAsset ("Assets/AstarPathfindingProject","Assets/Plugins/AstarPathfindingProject");
		AssetDatabase.Refresh ();
	}
	...
	public void DisableJs () {
		...
		if (System.IO.Directory.Exists (Application.dataPath+"/Plugins/AstarPathfindingProject")) {
			string error = AssetDatabase.MoveAsset ("Assets/Plugins/AstarPathfindingProject","Assets/AstarPathfindingProject");
			if (error != "") {
		...
		if (System.IO.Directory.Exists (Application.dataPath+"/AstarPathfindingEditor/Editor")) {
			string error = AssetDatabase.MoveAsset ("Assets/AstarPathfindingEditor/Editor","Assets/AstarPathfindingProject/Editor");
			if (error != "") {
				Debug.LogError ("Couldn't disable Js - "+error);
			} else {
				try {
	...
	public void DrawUserConnections () {
		...
		if (editLinks) {
			...
			if (Event.current.shift) {
				...
				if (node != null) {
					Handles.color = Color.yellow;
					Handles.SphereCap (GUIUtility.GetControlID (FocusType.Passive),node.position,Quaternion.identity,HandleUtility.GetHandleSize (node.position)*0.15F);
	...
	public void DrawUserConnectionsWindow () {
		...
		if (selectedUserConnection >= 0 && selectedUserConnection < conns.Length && Event.current.type != EventType.Used) {
			...
			if (Event.current.keyCode == KeyCode.Backspace && Event.current.type == EventType.KeyDown) {
				...
				HandleUtility.Repaint ();
				//Use the Event to avoid the annoying system beep (indicating a key which can't be used here, so we inform the system that it can)
				Event.current.Use ();
				GUI.changed = true;
				return;
			}
			...
			EditorGUIUtility.LookLikeControls (60,120);
			if (!conn.doOverrideCost) {
				
				GUI.enabled = false;
				EditorGUILayout.IntField ("Cost",((Int3)(conn.p1-conn.p2)).costMagnitude,GUILayout.MaxWidth (138));
			} else {
				
				conn.overrideCost = EditorGUILayout.IntField ("Cost",conn.overrideCost,GUILayout.MaxWidth (138));
			}
			GUI.enabled = true;
			
			GUILayout.FlexibleSpace ();
			conn.doOverrideCost = GUILayout.Toggle (conn.doOverrideCost,"");
			GUILayout.EndHorizontal ();
			
			conn.oneWay = EditorGUILayout.Toggle ("One Way",conn.oneWay);
	...
	public void DrawSerializationSettings () {
		...
		EditorGUILayout.Toggle (new GUIContent ("Cache startup","If enabled, will cache the graphs so they don't have to be scanned at startup.\nThis is an A* Pathfinding Project Pro only feature"),false);
		GUI.enabled = preEnabled;
		...
		if (GUILayout.Button ("Note that saving nodes is an A* Pathfinding Project Pro only feature. No nodes will be saved to file and no nodes can be loaded.\n"+
		                 "Therefore 'cache startup' is disabled since it, without node saving doesn't add anything extra. Click for more info",helpBox)) {
			Application.OpenURL (astarProInfoURL);
		}
		/*GUI.enabled = false;
	...
	[MenuItem ("Component/Pathfinding/Scan %&s")]
	public static void Scan () {
		...
		foreach (Progress progress in AstarPath.active.ScanLoop ()) {
			EditorUtility.DisplayProgressBar ("Scanning",progress.description,progress.progress);
		}
