...
namespace Pathfinding {
	...
	public class GridGraph : NavGraph, ISerializableGraph, IUpdatableGraph, IFunnelGraph
	{
		...
		public void GenerateMatrix () {
			...
			boundsMatrix.SetTRS (center,Quaternion.Euler (rotation),Vector3.one);
			...
			matrix.SetTRS (boundsMatrix.MultiplyPoint3x4 (-new Vector3 (size.x,0,size.y)*0.5F),Quaternion.Euler(rotation), new Vector3 (nodeSize,1,nodeSize));
		...
		public void CalculateConnections (GridNode[] graphNodes, int x, int z, GridNode node) {
		...
		public void UpdateArea (GraphUpdateObject o) {
			...
			if (!o.modifyWalkability && collision.collisionCheck) {
