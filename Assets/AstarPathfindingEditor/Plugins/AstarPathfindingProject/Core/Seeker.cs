...
public class Seeker : MonoBehaviour {
	...
	/** Called when a path has completed. This should have been implemented as optional parameter values, but that didn't seem to work very well with delegates (the values weren't the default ones)
	 * \see OnPathComplete(Path,bool,bool) */
	public void OnPathComplete (Path p) {
		OnPathComplete (p,true,true);
	}
	
	/** Called when a path has completed. Will post process it and return it by calling #tmpPathCallback and #pathCallback */
	...
	public Path StartPath (Vector3 start, Vector3 end, OnPathDelegate callback = null, int graphMask = -1) {
